...
describe("plugin entry source", () => {
  ...
  it("keeps package and plugin manifest versions in sync", () => {
    ...
    assert.equal(openclawVersion, "2026.9.4");
    assert.equal(pluginSdkVersion, "2026.9.4");
    assert.equal(openclawInstall.npmSpec, "openclaw-code-agent");
    assert.equal(openclawInstall.defaultChoice, "npm");
    assert.equal(openclawInstall.minHostVersion, ">=2026.9.4");
    ...
    assert.match(cliOutput, /openclaw\.install\.minHostVersion=>=2026\.9\.4/);
  ...
  it("requires OpenClaw 2026.9.4 for installation while retaining the verified 2026.8.1 API floor", () => {
    const packageJson = JSON.parse(readFileSync(join(rootDir, "package.json"), "utf8")) as {
      dependencies?: Record<string, string>;
      openclaw?: {
    ...
    assert.equal(packageJson.openclaw?.install?.minHostVersion, ">=2026.9.4");
    ...
    assert.equal(packageJson.openclaw?.build?.pluginSdkVersion, "2026.9.4");
    assert.equal(packageJson.peerDependencies?.openclaw, ">=2026.8.1");
    assert.equal(packageJson.devDependencies?.openclaw, "2026.9.4");
  ...
  it("accepts exact and range-shaped manual OpenClaw release targets", () => {
    assert.equal(normalizeOpenClawTargetVersion("2026.9.4"), "2026.9.4");
    assert.equal(normalizeOpenClawTargetVersion(">=2026.7.1"), "2026.7.1");
    assert.doesNotThrow(() =>
      validateReleaseMetadata({
        openclawTargetVersion: "2026.9.4",
  ...
  it("documents the 2026.9.4 compatibility and ownership boundaries", () => {
    const reference = readFileSync(join(rootDir, "docs", "REFERENCE.md"), "utf8");
    const readme = readFileSync(join(rootDir, "README.md"), "utf8");
    const changelog = readFileSync(join(rootDir, "CHANGELOG.md"), "utf8");

    assert.match(reference, /OpenClaw 2026\.9\.4 SDK Readiness/);
    assert.match(reference, /requires, is built against, and is validated against OpenClaw `2026\.9\.4`/);
    assert.match(readme, /requires, is built against, and is validated against OpenClaw `2026\.9\.4`/);
    assert.match(reference, /Package installation therefore requires `2026\.9\.4`/);
    assert.match(changelog, /against OpenClaw `2026\.9\.4`/i);
